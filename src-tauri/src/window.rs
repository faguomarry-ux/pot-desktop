...
fn get_current_monitor(x: i32, y: i32) -> Option<Monitor> {
    ...
    let monitors = daemon_window.available_monitors().unwrap_or_default();
...
fn translate_window() -> Window {
    ...
    // A hidden Wayland window need not have an assigned monitor yet.
    let monitor = window
        .current_monitor()
        .ok()
        .flatten()
        .or_else(|| get_current_monitor(mouse_position.x, mouse_position.y));
    let dpi = monitor
        .as_ref()
...
pub fn selection_translate() {
    ...
    let text = get_text();
    info!(
        "Selection translation received {} characters",
        text.chars().count()
    );
...
pub fn recognize_window() {
    ...
    let monitor = window
        ...
        .ok()
        .flatten()
        .or_else(|| get_current_monitor(0, 0));
    let dpi = monitor
        .as_ref()
        .map(Monitor::scale_factor)
        .or_else(|| window.scale_factor().ok())
...
pub fn ocr_recognize() {
    ...
    #[cfg(not(target_os = "macos"))]
    {
        ...
        window.listen_any("success", move |event| {
            recognize_window();
