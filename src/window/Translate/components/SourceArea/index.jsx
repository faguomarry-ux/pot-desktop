...
import { BaseDirectory, readTextFile } from '../../../../utils/tauri-fs.js';
...
import { detectBeforeTranslate } from '../../../../utils/detect-before-translate.js';
...
export default function SourceArea(props) {
    ...
    const keyDown = (event) => {
        // Enter must commit an IME composition before it can submit a translation.
    ...
    useEffect(() => {
        ...
        // WebKit reports zero scrollHeight while its native window is hidden.
        textarea.style.height = Math.max(50, textarea.scrollHeight) + 'px';
    }, [sourceText]);

    const detect_language = async (text) => {
        setDetectLanguage(await detectBeforeTranslate(detect, text));
    };
            ...
            >
                ...
                <CardBody className='bg-content1 p-[12px] pb-0 max-h-[40vh] overflow-y-auto'>
                    <textarea
                        autoFocus
                        ref={textAreaRef}
                        aria-label={t('config.hotkey.input_translate')}
                        rows={2}
                        className='bg-content1 w-full min-h-[50px] shrink-0 resize-none outline-none'
                        style={{ fontSize: `${appFontSize ?? 16}px` }}
                        value={sourceText}
                        onKeyDown={keyDown}
                        onChange={(e) => {
