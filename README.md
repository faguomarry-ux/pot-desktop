...
本仓库是 [faguomarry-ux/pot-desktop](https://github.com/faguomarry-ux/pot-desktop) 的原生编译链适配分支，基于 **[pot-app/pot-desktop 原项目](https://github.com/pot-app/pot-desktop)**。感谢原作者和所有贡献者提供的翻译器、界面设计、插件系统及跨平台实现；保留原项目 [GPL-3.0 许可证](LICENSE)。本分支主要维护 Tauri 2 / Ubuntu 26.04 适配、GNOME Wayland 运行修复和翻译接口修复，不代表上游官方发行。

### 本分支文档与下载

- [详细编译链迁移记录](docs/build-chain-migration.md)：依赖关系、Tauri 迁移、平台差异和验证范围。
- [Ubuntu 原生构建快速开始](docs/linux-native-build.md)：宿主依赖、编译及安装命令。
- [翻译服务与桌面修复记录](docs/service-repair-record.md)：Bing 接口更新、Google 429、输入与快捷键、代理设置。
- [CI 打包与发布说明](docs/ci-packaging.md)：x64/ARM64 DEB、RPM，以及 Intel/ARM64/Universal DMG。
- [本分支 Actions 构建产物](https://github.com/faguomarry-ux/pot-desktop/actions/workflows/package.yml) · [本分支 Releases](https://github.com/faguomarry-ux/pot-desktop/releases) · [本分支问题反馈](https://github.com/faguomarry-ux/pot-desktop/issues)。首次发布前请从成功的 Actions 运行下载产物。

Bing 已在 Ubuntu 原生 WebKit 下验证中英双向翻译；Google 当前测试返回 HTTP 429，详见修复记录。ARM64/macOS 是否构建通过请查看对应 CI 作业，不能仅依据 Linux x64 的本机结果判断。
...
本分支 Linux/macOS 包请使用上面的 Actions / Releases 链接，安装步骤见 [CI 打包说明](docs/ci-packaging.md)。下方 Winget、Homebrew、应用商店及扩展下载介绍继承自上游，安装的是原项目版本，不包含本分支修复。
...
1. 从 [Release](https://github.com/faguomarry-ux/pot-desktop/releases) 页面下载最新的 `dmg` 安装包。（如果您使用的是 M1 芯片，请下载名为`pot_{version}_aarch64.dmg`的安装包，否则请下载名为`pot_{version}_x64.dmg`的安装包）
...
1. 从 [Release](https://github.com/faguomarry-ux/pot-desktop/releases) 页面下载最新的对应架构的 `deb` 安装包。
...
使用 Node.js 24、pnpm 12.4.1、Rust 1.98.1 和 WebKitGTK 4.1：
...
此脚本用于 Ubuntu/Debian；macOS 与各架构命令见[详细编译链记录](docs/build-chain-migration.md)。开发模式为 `pnpm tauri dev`。不再安装旧的 `libwebkit2gtk-4.0-dev` / libsoup 2。
...
-   **[pot-app/pot-desktop](https://github.com/pot-app/pot-desktop.git)**：本项目的原作及代码基础，感谢原作者与社区贡献者；保留原有署名和 GPL-3.0 许可证。
...
请参阅 [Linux 原生构建说明](docs/linux-native-build.md)，使用 Tauri 2 / WebKitGTK 4.1 构建 Debian 包。
